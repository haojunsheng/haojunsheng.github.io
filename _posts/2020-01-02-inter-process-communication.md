...
<!-- Added by: anapodoton, at: 2020年 7月 4日 星期六 20时10分31秒 CST -->
...
关于select,poll,epoll可以参考[这个](https://github.com/haojunsheng/NetworkLearning/blob/master/unix-network-socket-api.md)。

# 3. 并发编程

如果逻辑控制流在时间上是重叠，那么它们就是`并发的(concurrent)`。这种常见的现象称为并发(concurrency)。

 我们主要将`并发`看做是一种操作系统内核用来运行多个应用程序的机制。但是，`并发`不仅仅局限于内核。它也可以在应用程序中扮演重要的角色。
